...
void IQGraphView::draw() {
    ...
    int theMVPMatrixID; glGetUniformLocation(theMVPMatrixID, "MVPMatrix");
    glUniformMatrix4fv(theMVPMatrixID, 1, false, glm::value_ptr(_fromToNDC));
    ...
    //-Setup vertex data
    int theVertexAttributeID; glGetAttribLocation(theVertexAttributeID, "vertexPosition");
    ...
    //Draw
#   ifdef RENDER_DEBUG
    __android_log_print(ANDROID_LOG_WARN, "IQ_APP", "Draw call. VBO id: [%d], elements count: [%d]",
                        _graphLayer->graphViewAccess_getVBOID(),
                        _graphLayer->graphViewAccess_getVBOVertexesNumber());
#   endif //RENDER_DEBUG
